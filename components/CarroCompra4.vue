        ...
        <ion-content padding>
            ...
            <ion-list>
                ...
                <div class="divclass">
                    ...
                    <ion-card v-for="carro of $store.getters.Cart" v-bind:key="carro.pp_codigo">
                        <ion-card-content v-if="carro.emp_run == $store.getters.Emprun && carro.suc_empresa_id == $store.getters.Sucempresaid">
                            <ion-card-title>{{carro.pp_nombre}}</ion-card-title>
        ...
        computed: {
            Cantidad(){
                ...
                for (var i=0; i < this.$store.getters.Cart.length; i++){
                    if (this.$store.getters.Emprun == this.$store.getters.Cart[i]['emp_run'] && this.$store.getters.Sucempresaid == this.$store.getters.Cart[i]['suc_empresa_id']){
                        cont = cont + 1;
                    }
                }
                return cont;
